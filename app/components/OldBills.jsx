...
export default class OldBills extends Component {
	...
	componentDidMount() {
		let _this = this;
		_this.props.dbBilling.find({}).limit(10).exec(function (err, data) {
			_this.setState({ oldBillTableData: data })
		});
	}
	handleViewOldBill(billID) {
		this.props.dbBilling.find({ stateTimeStamp: billID }, (err, data) => {
	...
	handleSearchID = (e) => {
		...
		this.props.dbBilling.find({ stateTimeStamp: this.refs.billID.value }, (err, data) => {
			this.setState({ oldBillWithID: data })
			console.log("dbID", this.state.oldBillMultiple, this.state.oldBillWithID)
		});
	}
	handleSearchPhone = (e) => {
		...
		this.props.dbBilling.find({ phoneNumber: this.refs.phoneNumber.value }, (err, data) => {
			if (data.length > 1) {
	...
	handleMultipleBill = (e) => {
		this.props.dbBilling.find({ stateTimeStamp: e.currentTarget.id }, (err, data) => {
			this.setState({ oldBillWithID: data })
					...
					<div className="container-fluid">
						...
						<div className="row">
							<div className="col-sm-4 offset-sm-2">
								<h3 className="search-bill-heading">Last 10 Bills</h3>
								<table className="table-striped padded-bottom-more">
									<thead>
										<tr>
											<th>SlNo</th>
									...
									<tbody>
										{this.state.oldBillTableData && this.state.oldBillTableData.map((data, index) => {
											return(
												<tr key={index}>
													<td>{index}</td>
													<td>{data.stateTimeStamp}</td>
													<td>{data.customerName}</td>
													<td>{data.phoneNumber}</td>
													<td><button className="btn btn-large btn-positive" onClick={this.handleViewOldBill.bind(this,data.stateTimeStamp)}>View</button></td>
												</tr>
											)
										})}
									</tbody>
